...
function TimeCell({
  ...
  const [value, setValue] = useState(
    currentValue ? inputTime(currentValue) : "",
  );
  ...
  if (!editing) {
    return (
      <button
        ...
        className="inline-flex items-center gap-1 text-xs text-on-surface-variant hover:text-primary disabled:cursor-default disabled:hover:text-on-surface-variant"
      >
  ...
  return (
    <div className="flex items-center gap-1">
      <input
        ...
        className="w-22 bg-surface-container-low rounded px-1 py-0.5 text-xs"
      />
      <button
        onClick={save}
        disabled={saving}
...
function BonusCell({
  ...
  if (!editing) {
    return (
      ...
      >
        <DollarSign size={11} />{" "}
        {Number(row.bonus) > 0 ? Number(row.bonus).toFixed(2) : "Add"}
      </button>
    );
  }
  return (
    <div className="flex items-center gap-1">
      <input
        ...
        value={value}
        onChange={(e) => setValue(e.target.value)}
        className="w-16 bg-surface-container-low rounded px-1.5 py-0.5 text-xs border border-outline-variant/20"
      />
      <button
        onClick={save}
        disabled={saving}
...
export default function DailyAttendance() {
  ...
  async function markOnTime(row: AttendanceRow, field: "checkIn" | "checkOut") {
    const value =
      field === "checkIn" ? row.scheduleStartTime : row.scheduleEndTime;
    if (!value) return;
    setBusyStaffId(row.staffId);
    try {
      await updateAttendanceTimes(row.staffId, date, { [field]: value });
      await load();
    } finally {
      setBusyStaffId(null);
    }
  ...
  async function handleOpenShiftToggle(row: AttendanceRow) {
    ...
    try {
      await toggleOpenShiftAttendance({
        staffId: row.staffId,
        date,
        attended: !row.openShiftAttended,
        openShiftAssignmentId: row.openShiftAssignmentId,
      });
      await load();
    } finally {
      setBusyStaffId(null);
  ...
  const totalHours = rows.reduce(
    (s, r) => s + Number(r.regularHours ?? 0) + Number(r.openShiftHours ?? 0),
    0,
  );
        ...
        <header className="mb-6 flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
          <div>
            ...
            <p className="text-on-surface-variant text-sm mt-1">
              Check staff in and out, and record any open-shift work or bonuses.
            </p>
          </div>
          <div className="flex flex-wrap items-center justify-end gap-2">
            ...
            <input
              ...
              className="bg-surface-container-low rounded-lg px-3 py-2 text-sm border border-transparent focus:border-primary/30 focus:outline-none"
            />
            <button
              onClick={() => markAllOnTime("checkIn")}
              className="rounded-lg bg-primary/10 px-3 py-2 text-xs font-semibold text-primary hover:bg-primary/20"
            >
              On time all Start
            </button>
            <button
              onClick={() => markAllOnTime("checkOut")}
              className="rounded-lg bg-tertiary/10 px-3 py-2 text-xs font-semibold text-tertiary hover:bg-tertiary/20"
            >
              On time all End
            </button>
          </div>
            ...
            <table className="w-full text-left border-collapse text-sm">
              ...
              <tbody>
                {rows.map((row) => {
                  ...
                  const totalHrs =
                    Number(row.regularHours ?? 0) +
                    Number(row.openShiftHours ?? 0);
                  return (
                    <tr
                      key={row.staffId}
                    ...
                    >
                      <td className="py-2.5 pr-4">
                        ...
                        <p className="text-[10px] text-on-surface-variant">
                          {row.scheduleLabel ?? row.staffRole} •{" "}
                      ...
                      <td className="py-2.5 pr-3 text-center">
                        {row.checkIn ? (
                          <div className="flex flex-col items-center gap-1">
                            <TimeCell
                              key={`${row.staffId}-start-${row.checkIn}`}
                              row={row}
                              date={date}
                              field="checkIn"
                              onSaved={load}
                            ...
                            {row.scheduleStartTime && (
                              <button
                                disabled={busy}
                                onClick={() => markOnTime(row, "checkIn")}
                          ...
                          <div className="flex flex-col items-center gap-1">
                            <button
                              disabled={busy}
                              onClick={() => handleCheckIn(row.staffId)}
                              className="w-5 h-5 rounded border border-outline-variant/40 hover:border-primary hover:bg-primary/10 transition-colors inline-flex items-center justify-center disabled:opacity-40"
                              title="Mark attendance start"
                            />
                            {row.scheduleStartTime && (
                              <button
                                disabled={busy}
                      ...
                      <td className="py-2.5 pr-3 text-center">
                        {row.checkOut ? (
                          <div className="flex flex-col items-center gap-1">
                            <TimeCell
                              key={`${row.staffId}-end-${row.checkOut}`}
                              row={row}
                              date={date}
                              field="checkOut"
                              onSaved={load}
                            ...
                            {row.scheduleEndTime && (
                              <button
                                disabled={busy}
                                onClick={() => markOnTime(row, "checkOut")}
                                className="text-[9px] font-semibold text-tertiary hover:underline"
                              >
                                On time
                              </button>
                            )}
                          ...
                          <div className="flex flex-col items-center gap-1">
                            <button
                              disabled={busy}
                              onClick={() => handleCheckOut(row.staffId)}
                              className="w-5 h-5 rounded border border-outline-variant/40 hover:border-primary hover:bg-primary/10 transition-colors inline-flex items-center justify-center disabled:opacity-40"
                              title="Mark attendance end"
                            />
                            {row.scheduleEndTime && (
                              <button
                                disabled={busy}
                          ...
                          <div className="flex flex-col items-center gap-1">
                            <button
                              ...
                              className={`w-5 h-5 rounded border inline-flex items-center justify-center transition-colors disabled:opacity-40 ${
                                row.openShiftAttended
                                  ? "bg-tertiary border-tertiary text-on-tertiary"
                                  : "border-outline-variant/40 hover:border-tertiary hover:bg-tertiary/10"
                              }`}
                            >
                      ...
                      <td className="py-2.5 pr-3 text-right text-on-surface">
                        {totalHrs > 0 ? totalHrs.toFixed(1) : "—"}
                      </td>
                      <td className="py-2.5 pr-3 text-right text-on-surface-variant">
                        {row.regularWage
                          ? `$${Number(row.regularWage).toFixed(2)}`
                          : "—"}
                      </td>
                      <td className="py-2.5 pr-3 text-right text-on-surface-variant">
                        {row.openShiftWage
                          ? `$${Number(row.openShiftWage).toFixed(2)}`
                          : "—"}
                      </td>
                      <td className="py-2.5 pr-3 text-right">
                        <BonusCell row={row} date={date} onSaved={load} />
                      </td>
                      <td className="py-2.5 pr-0 text-right font-semibold text-primary">
                        {row.totalWage
                          ? `$${Number(row.totalWage).toFixed(2)}`
                          : "—"}
                      </td>
                    </tr>
                  );
