...
export default function GalleryAdminManagement() {
  ...
  async function addImage() {
    if (!form.title.trim() || !form.imageUrl.trim())
      return setMessage("Title and image URL are required.");
    try {
      ...
      <div className="mx-auto max-w-6xl">
        <header className="border-b border-outline-variant/20 pb-6">
          ...
          <p className="mt-2 text-sm text-secondary">
            Add images by URL or upload them for the Noir Dining gallery.
          </p>
        </header>
        {message && <p className="mt-5 text-sm text-tertiary">{message}</p>}
        <section className="mt-8 rounded-xl border border-outline-variant/20 bg-surface-container-lowest p-5">
          ...
          <div className="mt-5 grid gap-4 md:grid-cols-2">
            <input
              ...
              className="rounded-lg border border-outline-variant/25 bg-surface px-3 py-2 text-sm outline-none focus:border-primary"
            />
            <select
              value={form.category}
              onChange={(event) =>
            ...
            <input
              ...
              className="rounded-lg border border-outline-variant/25 bg-surface px-3 py-2 text-sm outline-none focus:border-primary md:col-span-2"
            />
            <input
              value={form.altText}
              onChange={(event) =>
          ...
          <div className="mt-5 flex flex-wrap items-center gap-3">
            <label className="inline-flex cursor-pointer items-center gap-2 rounded-lg border border-outline-variant/25 px-4 py-2 text-xs font-bold uppercase tracking-wider text-secondary">
            ...
            <button
              ...
              className="rounded-lg bg-primary px-5 py-2.5 text-xs font-bold uppercase tracking-wider text-white"
            >
              Add image
            </button>
          </div>
        ...
        <section className="mt-8">
          {loading ? (
            <p className="py-10 text-sm text-secondary">Loading gallery...</p>
          ) : (
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {images.map((image) => (
                <article
                  key={image.id}
                  className={`overflow-hidden rounded-xl border border-outline-variant/20 bg-surface-container-lowest ${!image.isActive ? "opacity-55" : ""}`}
                >
                  ...
                  <div className="p-4">
                    ...
                    <h3 className="mt-1 font-headline text-lg text-primary">
                      {image.title}
                    </h3>
