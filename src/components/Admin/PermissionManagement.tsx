...
import { getAccessGrants, getCurrentUser, removeAccess, requestAccess, reviewAccess, type AccessGrant, type AccessModule } from "../../api/authorization";
...
export default function PermissionManagement() {
  ...
  async function remove(id: string, label: string) {
    if (!window.confirm(`Remove ${label} access? This only removes this permission.`)) return;
    try { await removeAccess(id); setMessage(`${label} access removed`); await load(); }
    catch (error) { setMessage(error instanceof Error ? error.message : "Failed to remove access"); }
  }
