...
export default function PermissionManagement() {
  ...
  async function load() {
    try {
      const [current, grantResponse, staffResponse] = await Promise.all([
        getCurrentUser(),
        getAccessGrants(),
        getStaffList(),
      ]);
  ...
  async function submitRequest() {
    ...
    try {
      await requestAccess(userId, module);
  ...
  return (
    <section className="p-6 md:p-10 space-y-6">
      <div>
        ...
        <h1 className="text-3xl font-headline font-bold mt-1">
          Permission Management
        </h1>
        <p className="text-sm text-on-surface-variant mt-2">
          Grant staff access to operational modules. Manager requests require
          Admin approval.
        </p>
      </div>
      {message && (
      ...
      <div className="bg-surface-container-lowest rounded-xl p-5 border border-outline-variant/15 space-y-4">
        <h2 className="font-semibold flex items-center gap-2">
          <Shield size={17} className="text-primary" /> Request module access
        </h2>
        <div className="flex flex-col md:flex-row gap-3">
          <select
            value={userId}
            onChange={(event) => setUserId(event.target.value)}
            className="bg-surface-container-low rounded-lg px-3 py-2 text-sm flex-1"
          >
          ...
          <select
            value={module}
            onChange={(event) => setModule(event.target.value as AccessModule)}
            className="bg-surface-container-low rounded-lg px-3 py-2 text-sm"
          >
            {MODULES.map((item) => (
          ...
          >
            {userRole === "Admin" ? "Grant access" : "Request access"}
          </button>
        </div>
      </div>
              ...
              >
                <td className="p-4">
                  <p className="font-semibold">
                    {grant.user.name || "Unnamed"}
                  </p>
                  <p className="text-xs text-on-surface-variant">
                    {grant.user.email}
                  </p>
                </td>
                <td className="p-4">{grant.module}</td>
                <td className="p-4">
                  <span
                    className={`text-xs font-semibold ${grant.status === "APPROVED" ? "text-primary" : grant.status === "REJECTED" ? "text-error" : "text-secondary"}`}
                  >
                    {grant.status}
                  </span>
                </td>
                <td className="p-4">
                  <div className="flex gap-1">
                    {userRole === "Admin" && grant.status === "PENDING" && (
                      <>
                        <button
                          title="Approve"
                          onClick={() => void review(grant.id, "APPROVED")}
                        ...
                        <button
                          title="Reject"
                          onClick={() => void review(grant.id, "REJECTED")}
                    ...
                    {userRole === "Admin" && (
                      <button
                        title="Remove access"
                        onClick={() => void remove(grant.id, grant.module)}
                        className="p-2 text-error hover:bg-error/10 rounded-lg"
                      >
                        <Trash2 size={15} />
                      </button>
