  ...
  return (
    <article className="receipt-paper mx-auto w-full max-w-sm bg-white px-6 py-7 text-[#1b1c1a] shadow-lg">
      <header className="border-b border-dashed border-[#8a8a84] pb-4 text-center">
        <p className="text-[10px] font-bold uppercase tracking-[0.25em] text-[#625e54]">The Culinary Editorial</p>
        <h2 className="mt-1 font-serif text-2xl font-bold">Order Receipt</h2>
        <p className="mt-2 text-xs text-[#625e54]">
          Order #{String(order.orderNumber).padStart(5, "0")} · {order.orderType.replace("_", "-")}
        </p>
      </header>

      <div className="border-b border-dashed border-[#8a8a84] py-3 text-xs text-[#625e54]">
        ...
        {order.items.map((item, index) => (
          <div key={`${item.menuItem?.name ?? "item"}-${index}`} className="flex justify-between gap-4 text-sm">
            <div className="min-w-0">
              ...
              <p className="text-xs text-[#625e54]">{item.quantity} × ${money(item.unitPrice)}</p>
              {item.note && <p className="text-xs italic text-[#625e54]">{item.note}</p>}
            </div>
      ...
      <div className="space-y-1 py-4 text-xs text-[#625e54]">
      ...
      <footer className="border-t border-dashed border-[#8a8a84] pt-4 text-center text-[10px] uppercase tracking-[0.16em] text-[#625e54]">
